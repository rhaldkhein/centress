...
function () {
  function Server(provider, options) {
    ...
    this.core = provider.service('__core__'); // Create app instances
    ...
    this.appRoot.use(function (req, res, next) {
      ...
      req.provider = _this.core.createScopedProvider();
      next();
    });
    debugServer('created');
  ...
  _createClass(Server, [{
    ...
    value: function listen() {
      ...
      this.appRoot.use(this.core.init()); // Attach primary routers
    ...
    value: function start(provider) {
      var server = provider.service('@server');
