...
function () {
  ...
  _createClass(Server, [{
    key: "attachApiRoutes",
    value: function attachApiRoutes() {
      // Attach primary routers
      this.appRoot.use(this.config.apiBaseUrl, this.appApi); // Last middleware for API
    ...
    value: function listen() {
      ...
      var httpsPort = this.config.portSecure || parseInt(httpPort) + 1; // Infuse di container to request

      this.appRoot.use(this.core.init(_http.IncomingMessage.prototype));
      ...
      if (result) return result.then(runServer);else runServer();
    }
  }]);
