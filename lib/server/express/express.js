...
_express.application.useServiceRoutes = function () {
  ...
  var collection = this.$provider.service('core').collection; // Apply filters

  var includes = options.includes,
      excludes = options.excludes;
  var services = collection.services;
  if (includes) services = services.filter(function (s) {
    return includes.indexOf(s.name) > -1;
  });
  if (excludes) services = services.filter(function (s) {
    return !(excludes.indexOf(s.name) > -1);
